...
### Update
### Update
Run the unified deployment script (handles git pull, pip install, restart):
```bash
./deploy/deploy.sh
```

### Docker (Recommended)
Docker support is coming in Phase 5 to simplify environment management.
For now, use `deploy.sh`.
