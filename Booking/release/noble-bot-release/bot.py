...
NAME, PHONE, DATE, TIME, PARTY_SIZE, COMMENT, CONFIRMATION = range(7)
# Состояния для отзывов
REVIEW_TEXT, REVIEW_CONTACT = range(7, 9)
# Состояния для админов
ADMIN_CHOICE, ADMIN_INPUT_ADD, ADMIN_INPUT_DEL = range(9, 12)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"User started bot: {user.id} ({user.first_name})") # Логируем ID
    welcome = f"""
...
async def admin_show_bookings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bookings = ReservationRepository.get_upcoming(limit=20)
    if not bookings:
