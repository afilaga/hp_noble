...
7.  **End:** Saves to DB, shows "✅ Created", returns to Main Menu.
8.  **Fallbacks:** Includes Main Menu buttons ("🌐 Сайт", etc.) to prevent hanging if user switches context mid-booking.
...
*   **Features:**
    *   **Bookings:** "📅 Актуальные брони" (shows upcoming bookings list).
...
1.  **Kill old process:** `pkill -9 -f "python3 bot.py"` (Use -9 for hard kill to prevent zombies).
2.  **Navigate:** `cd /Users/andreyfilatiev/Documents/Hookah\ Place\ Noble/Site/модуль`
3.  **Run:** `python3 bot.py`
4.  **Logs:** Check console/logfile for `[INFO] Бот запущен!`.
...
*   [ ] Integration with Google Calendar (via Service Account).
